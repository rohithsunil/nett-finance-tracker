...
async function fetchLiveRates() {
  ...
  for (const base of ['AED', 'INR', 'USD', 'EUR', 'GBP']) for (const quote of ['AED', 'INR', 'USD', 'EUR', 'GBP']) if (base !== quote) { const rate = pair(base, quote); if (rate) result[`${base}_${quote}`] = rate; }
  return result;
}
...
export default function NettApp() {
  ...
  const scopedBase = (workspace === 'everything' ? data : {
    ...data,
  ...
  async function saveAccount(form: HTMLFormElement) {
    ...
    const previousCard = data.creditCards.find((card) => card.account_id === accountId);
    ...
    if (supabase && session) {
      ...
      if (item.type === 'credit_card') {
        const cardResult = await supabase.from('credit_cards').upsert({ id: previousCard?.id || crypto.randomUUID(), user_id: session.userId, account_id: item.id, credit_limit: Number(formData.get('credit_limit') || 0), current_outstanding: Number(formData.get('current_outstanding') || 0), statement_balance: Number(formData.get('statement_balance') || 0), statement_date: String(formData.get('statement_date') || '') || null, payment_due_date: String(formData.get('payment_due_date') || '') || null, minimum_payment: Number(formData.get('minimum_payment') || 0) }, { onConflict: 'account_id' });
        if (cardResult.error) { notify(`Account saved, but card details failed: ${cardResult.error.message}`); return; }
      }
    }
    const nextCard = item.type === 'credit_card' ? { id: previousCard?.id || crypto.randomUUID(), account_id: item.id, credit_limit: Number(formData.get('credit_limit') || 0), current_outstanding: Number(formData.get('current_outstanding') || 0), statement_balance: Number(formData.get('statement_balance') || 0), statement_date: String(formData.get('statement_date') || '') || null, payment_due_date: String(formData.get('payment_due_date') || '') || null, minimum_payment: Number(formData.get('minimum_payment') || 0) } as CreditCardRecord : null;
    setData((current) => ({ ...current, accounts: previous ? current.accounts.map((account) => account.id === item.id ? item : account) : [...current.accounts, item], creditCards: nextCard ? (previousCard ? current.creditCards.map((card) => card.account_id === item.id ? nextCard : card) : [...current.creditCards, nextCard]) : current.creditCards }));
  ...
  async function saveCommitment(form: HTMLFormElement) {
    const values = new FormData(form);
    const item: Commitment = { id: editingCommitment?.id || crypto.randomUUID(), workspace_id: String(values.get('workspace_id')), name: String(values.get('name')).trim(), amount: Number(values.get('amount')), currency: String(values.get('currency')), due_date: String(values.get('due_date')), recurrence: String(values.get('recurrence')), importance: String(values.get('importance')) as Commitment['importance'], status: editingCommitment?.status || 'open', expected_income: values.get('expected_income') === 'true', confidence: String(values.get('confidence') || 'confirmed'), country_code: String(values.get('country_code') || 'AE'), notes: String(values.get('notes') || '') || null };
    const supabase = getSupabaseBrowser();
    if (supabase && session) {
      const payload = { user_id: session.userId, ...item };
      const result = editingCommitment ? await supabase.from('commitments').update(payload).eq('id', item.id).eq('user_id', session.userId) : await insertWithLegacyFallback(supabase, 'commitments', payload, { ...payload, country_code: undefined });
      if (result.error && isMissingFinancialMigration(result.error) && editingCommitment) { const fallback = await supabase.from('commitments').update({ ...payload, country_code: undefined }).eq('id', item.id).eq('user_id', session.userId); if (fallback.error) { notify(`Could not save commitment: ${fallback.error.message}`); return; } } else if (result.error) { notify(`Could not save commitment: ${result.error.message}`); return; }
    }
    ...
    setModal(null); setEditingCommitment(null); notify(`${item.name} ${editingCommitment ? 'updated' : 'added to your future timeline'}.`);
  }
  ...
  async function saveDebt(form: HTMLFormElement) {
    const values = new FormData(form);
    const item: Debt = { id: editingDebt?.id || crypto.randomUUID(), workspace_id: String(values.get('workspace_id')), name: String(values.get('name')).trim(), debt_class: String(values.get('debt_class')) as Debt['debt_class'], original_principal: Number(values.get('original_principal')), outstanding: Number(values.get('outstanding')), currency: String(values.get('currency')), comfortable_target: Number(values.get('comfortable_target') || 0) || null, due_date: String(values.get('due_date') || '') || null, status: editingDebt?.status || 'open', country_code: String(values.get('country_code') || 'AE'), notes: String(values.get('notes') || '') || null };
    if (!session) { notify('Sign in before saving a debt.'); return; }
    const supabase = getSupabaseBrowser();
    if (supabase) { const payload = { user_id: session.userId, ...item }; const result = editingDebt ? await supabase.from('debts').update(payload).eq('id', item.id).eq('user_id', session.userId) : await insertWithLegacyFallback(supabase, 'debts', payload, { ...payload, country_code: undefined }); if (result.error && isMissingFinancialMigration(result.error) && editingDebt) { const fallback = await supabase.from('debts').update({ ...payload, country_code: undefined }).eq('id', item.id).eq('user_id', session.userId); if (fallback.error) { notify(`Could not save debt: ${fallback.error.message}`); return; } } else if (result.error) { notify(`Could not save debt: ${result.error.message}`); return; } }
    setData((current) => ({ ...current, debts: [...current.debts.filter((entry) => entry.id !== item.id), item] })); setModal(null); setEditingDebt(null); notify(`${item.name} ${editingDebt ? 'updated' : 'added to your debt plan'}.`);
  }
  ...
  async function createReceivableEvent(form: HTMLFormElement) {
    ...
    if (supabase) { const rpc = await supabase.rpc('nett_apply_receivable_event', { p_user_id: session.userId, p_receivable_id: receivableId, p_event_type: nextOutstanding === 0 ? 'settlement' : 'repayment', p_amount: amount, p_currency: target.currency, p_destination_account_id: String(values.get('destination_account_id') || '') || null, p_note: String(values.get('note') || ''), p_occurred_at: new Date().toISOString() }); if (rpc.error && !isMissingFinancialMigration(rpc.error)) { notify(`Could not save receivable payment: ${rpc.error.message}`); return; } if (rpc.error && isMissingFinancialMigration(rpc.error)) { const eventInsert = await supabase.from('receivable_events').insert({ user_id: session.userId, receivable_id: receivableId, event_type: nextOutstanding === 0 ? 'settlement' : 'repayment', amount, currency: target.currency, destination_account_id: String(values.get('destination_account_id') || '') || null, note: String(values.get('note') || '') }); if (eventInsert.error) { notify(`Could not save receivable payment: ${eventInsert.error.message}`); return; } const itemUpdate = await supabase.from('receivables').update({ outstanding: nextOutstanding, status: nextOutstanding === 0 ? 'settled' : 'open' }).eq('id', receivableId).eq('user_id', session.userId); if (itemUpdate.error) { notify(`Payment saved, but receivable update failed: ${itemUpdate.error.message}`); return; } } }
    setData((current) => ({ ...current, receivables: current.receivables.map((item) => item.id === receivableId ? { ...item, outstanding: nextOutstanding, status: nextOutstanding === 0 ? 'settled' : 'open' } : item) })); setModal(null); notify('Receivable payment recorded.');
  }
  ...
  async function createInvestment(form: HTMLFormElement) {
    const values = new FormData(form); const valueAt = new Date().toISOString(); const item: Investment = { id: crypto.randomUUID(), workspace_id: String(values.get('workspace_id')), symbol: String(values.get('symbol')).toUpperCase(), exchange: String(values.get('exchange')), name: String(values.get('name')), quantity: Number(values.get('quantity')), holding_currency: String(values.get('currency')), average_cost: Number(values.get('average_cost')), liquid: values.get('liquid') === 'true', latest_value: Number(values.get('value')), latest_value_at: valueAt, latest_value_source: 'manual', country_code: String(values.get('country_code') || 'AE') };
    const supabase = getSupabaseBrowser();
    if (supabase && session) { let investmentInsert = await supabase.from('investments').insert({ id: item.id, user_id: session.userId, workspace_id: item.workspace_id, symbol: item.symbol, exchange: item.exchange, name: item.name, quantity: item.quantity, holding_currency: item.holding_currency, average_cost: item.average_cost, liquid: item.liquid, country_code: item.country_code }).select('id').single(); if (isMissingFinancialMigration(investmentInsert.error)) investmentInsert = await supabase.from('investments').insert({ id: item.id, user_id: session.userId, workspace_id: item.workspace_id, symbol: item.symbol, exchange: item.exchange, name: item.name, quantity: item.quantity, holding_currency: item.holding_currency, average_cost: item.average_cost, liquid: item.liquid }).select('id').single(); const { data: created, error } = investmentInsert; if (error || !created) { notify(`Could not save holding: ${error?.message || 'unknown error'}`); return; } const valueResult = await supabase.from('investment_values').insert({ user_id: session.userId, investment_id: created.id, value: item.latest_value, currency: item.holding_currency, source: 'manual', valued_at: valueAt }); if (valueResult.error) { notify(`Holding saved, but valuation failed: ${valueResult.error.message}`); return; } }
    setData((current) => ({ ...current, investments: [...current.investments, item], investmentValues: [...current.investmentValues, { id: crypto.randomUUID(), investment_id: item.id, value: item.latest_value || 0, currency: item.holding_currency, source: 'manual', valued_at: valueAt }] }));
  ...
  return <div className="app-shell">
    <aside className="desktop-sidebar">
      ...
      <nav className="nav-list">{navItems.map(({ id, label, icon: Icon }) => <button key={id} className={`nav-button ${tab === id ? 'active' : ''}`} onClick={() => setTab(id)}><Icon size={17} strokeWidth={tab === id ? 2.2 : 1.8} /><span className="nav-caption">{label}</span>{id === 'plan' && (scoped.debts.length + scoped.commitments.length > 0) && <span style={{ marginLeft: 'auto', color: '#bd7dd7', fontSize: 10 }}>{scoped.debts.length + scoped.commitments.length}</span>}</button>)}</nav>
      ...
      <select value={country} onChange={(event) => setCountry(event.target.value)} aria-label="Country filter" style={{ fontSize: 11, padding: '9px 10px', margin: '7px 3px 0', width: 'calc(100% - 6px)' }}><option value="all">All countries</option><option value="AE">🇦🇪 UAE</option><option value="IN">🇮🇳 India</option><option value="US">🇺🇸 United States</option><option value="GB">🇬🇧 United Kingdom</option></select>
      <div className="context-row"><span>Totals in</span><select value={displayCurrency} onChange={(event) => void updateDisplayCurrency(event.target.value)} aria-label="Display currency"><option>AED</option><option>INR</option><option>USD</option><option>EUR</option><option>GBP</option></select></div>
      <button className="nav-button compact-action" onClick={() => setModal('workspace')}><Plus size={15} /><span className="nav-caption">New workspace</span></button>
    ...
    <main className="main">
      ...
      {tab === 'accounts' && <AccountsViewV3 data={scoped} displayCurrency={comparisonCurrency} onQuick={(next) => setModal(next)} onEdit={(account) => { setEditingAccount(account); setModal('account'); }} onMove={(account) => { setMovingAccount(account); setModal('move-account-country'); }} onDelete={(account) => { setDeletingAccount(account); setModal('delete-account'); }} />}
      ...
      {tab === 'plan' && <PlanViewV2 data={scoped} metrics={metrics} displayCurrency={displayCurrency} onQuick={(next) => setModal(next)} onEditCommitment={(item) => { setEditingCommitment(item); setModal('commitment'); }} onDeleteCommitment={(item) => { setDeletingCommitment(item); setModal('delete-commitment'); }} onEditDebt={(item) => { setEditingDebt(item); setModal('debt'); }} onDeleteDebt={(item) => { setDeletingDebt(item); setModal('delete-debt'); }} whatIf={whatIf} setWhatIf={setWhatIf} whatIfResult={whatIfResult} runWhatIf={runWhatIf} />}
      {tab === 'plan' && <PlanManagePanel data={scoped} onEditCommitment={(item) => { setEditingCommitment(item); setModal('commitment'); }} onDeleteCommitment={(item) => { setDeletingCommitment(item); setModal('delete-commitment'); }} onEditDebt={(item) => { setEditingDebt(item); setModal('debt'); }} onDeleteDebt={(item) => { setDeletingDebt(item); setModal('delete-debt'); }} />}
    ...
    {modal === 'account' && <AccountModal account={editingAccount} card={editingAccount ? data.creditCards.find((item) => item.account_id === editingAccount.id) || null : null} workspaces={data.workspaces} onClose={() => { setModal(null); setEditingAccount(null); }} onSave={saveAccount} onDelete={(account) => { setEditingAccount(null); setDeletingAccount(account); setModal('delete-account'); }} />}
    ...
    {modal === 'commitment' && <CommitmentEditorModal commitment={editingCommitment} workspaces={data.workspaces} onClose={() => { setModal(null); setEditingCommitment(null); }} onSave={saveCommitment} />}
...
function AccountsViewV3({ data, displayCurrency, onQuick, onEdit, onMove, onDelete }: { data: NettData; displayCurrency: string; onQuick: (modal: Modal) => void; onEdit: (account: Account) => void; onMove: (account: Account) => void; onDelete: (account: Account) => void }) {
  ...
  return <div className="page-panel">
    <div className="view-header"><div><h2>Accounts & cards</h2><p>Every balance, identity detail and due date in one calm view.</p></div><div className="action-row"><button className="soft-button" onClick={() => onQuick('transfer')}><MoveRight size={15} /> Move money</button><button className="primary-button" onClick={() => onQuick('account')}><Plus size={16} /> Add account</button></div></div>
     <section className="card full-card accounts-board"><div className="accounts-board-note"><div><strong>{data.accounts.length} account{data.accounts.length === 1 ? '' : 's'}</strong><span>Balances are shown in their original currency. Converted totals use the rate above.</span></div><span className="status-chip"><CircleDollarSign size={13} /> FX context above</span></div><div className="account-detail-grid">{data.accounts.map((account) => <article className="account-detail-card" key={account.id}><div className="account-detail-top"><div className="account-heading"><AccountLogo account={account} size={42} /><div><div className="account-detail-name">{account.name}</div><div className="table-muted">{account.institution_name || 'Personal account'} · {account.type.replace('_', ' ')}</div></div></div><span className={`pill ${isStale(account.balance_verified_at, data.profile.freshness_days) ? 'warn' : 'good'}`}>{isStale(account.balance_verified_at, data.profile.freshness_days) ? 'Update needed' : 'Verified'}</span></div><div className="table-muted account-meta-line">{countryLabel(account.country_code || 'AE')} · {account.currency}{account.account_last4 ? ` · •••• ${account.account_last4}` : ''} · Checked {account.balance_verified_at ? formatShortDate(account.balance_verified_at) : 'never'}</div><div className="account-balance-row"><div className="account-detail-balance">{formatCurrency(Number(account.estimated_balance ?? account.verified_balance), account.currency)}</div><div className="account-converted-balance">≈ {formatCurrency(displayAmount(Number(account.estimated_balance ?? account.verified_balance), account.currency, displayCurrency, data.fxRates), displayCurrency)}</div></div><div className="account-detail-actions"><span className="pill">{account.include_net_worth ? 'In net worth' : 'Excluded'}</span><span className="pill">{account.include_liquidity ? 'Liquid' : 'Not liquid'}</span><div className="account-card-spacer" /><button className="edit-account-button" onClick={() => onMove(account)}><MoveRight size={13} /> Move</button><button className="edit-account-button" onClick={() => onEdit(account)}><Pencil size={13} /> Edit</button><button className="edit-account-button account-delete-icon" onClick={() => onDelete(account)} aria-label={`Delete ${account.name}`} title="Delete account"><Trash2 size={13} /></button></div></article>)}</div>{!data.accounts.length && <div className="empty-state"><Wallet size={20} /><strong>Your accounts will appear here</strong><span>Add current accounts, cash, wallets or credit cards.</span></div>}</section>
...
function MoreView({ data, theme, setTheme, pushEnabled, enableNotifications, exportData, exportCsv, exportXlsx, session, onQuick }: { data: NettData; theme: Theme; setTheme: (theme: Theme) => void; pushEnabled: boolean; enableNotifications: () => void; exportData: () => void; exportCsv: () => void; exportXlsx: () => void; session: boolean; onQuick: (modal: Modal) => void }) {
...
function AccountModal({ account, card, workspaces, onClose, onSave, onDelete }: { account: Account | null; card: CreditCardRecord | null; workspaces: NettData['workspaces']; onClose: () => void; onSave: (form: HTMLFormElement) => void; onDelete: (account: Account) => void }) {
...
function MoveAccountCountryModal({ account, onClose, onSave }: { account: Account; onClose: () => void; onSave: (form: HTMLFormElement) => void }) {
  return <ModalShell title={`Move ${account.name}`} description="This changes the country used for net-worth and activity views. Existing activity linked to this account will follow the new country." onClose={onClose}>
    <form onSubmit={(event) => { event.preventDefault(); onSave(event.currentTarget); }}>
      ...
      <label>Move to<select name="country_code" defaultValue={account.country_code || 'AE'}>{countryOptions.map((item) => <option key={item.value} value={item.value}>{item.flag} {item.label}</option>)}</select></label>
