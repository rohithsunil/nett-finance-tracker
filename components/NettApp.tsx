...
export default function NettApp() {
  ...
  async function createDebtEvent(form: HTMLFormElement) {
    ...
    setData((current) => ({ ...current, debts: current.debts.map((item) => item.id === debtId ? { ...item, outstanding, status: outstanding === 0 ? 'settled' : 'open' } : item) }));
  ...
  return <div className="app-shell">
    ...
    {modal === 'debt-event' && <DebtModal debts={data.debts} accounts={data.accounts} onClose={() => setModal(null)} onSave={createDebtEvent} />}
...
function AccountsViewV3({ data, displayCurrency, onQuick, onEdit, onMove }: { data: NettData; displayCurrency: string; onQuick: (modal: Modal) => void; onEdit: (account: Account) => void; onMove: (account: Account) => void }) {
