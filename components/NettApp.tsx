...
export default function NettApp() {
  ...
  return <div className="app-shell">
    ...
    {modal === 'debt' && <DebtCreateModal workspaces={data.workspaces} onClose={() => setModal(null)} onSave={createDebt} />}
...
function CommitmentModal({ workspaces, onClose, onSave }: { workspaces: NettData['workspaces']; onClose: () => void; onSave: (form: HTMLFormElement) => void }) { return <CommitmentModalV2 workspaces={workspaces} onClose={onClose} onSave={onSave} />; }
...
function DebtCreateModal({ workspaces, onClose, onSave }: { workspaces: NettData['workspaces']; onClose: () => void; onSave: (form: HTMLFormElement) => void }) { return <ModalShell title="Add a debt or loan" description="Track what you owe, whether it is a formal loan or a flexible IOU." onClose={onClose}><form onSubmit={(event) => { event.preventDefault(); onSave(event.currentTarget); }}><div className="form-grid"><label className="full-span">Name<input name="name" required placeholder="Car loan, family IOU…" /></label><label>Type<select name="debt_class" defaultValue="mandatory"><option value="mandatory">Mandatory loan</option><option value="flexible">Flexible IOU</option></select></label><label>Currency<select name="currency" defaultValue="AED"><option>AED</option><option>INR</option><option>USD</option><option>EUR</option><option>GBP</option></select></label><label>Original principal<input name="original_principal" required type="number" min="0.01" step="0.01" placeholder="0.00" /></label><label>Current outstanding<input name="outstanding" required type="number" min="0" step="0.01" placeholder="0.00" /></label><label>Comfortable payment<input name="comfortable_target" type="number" min="0" step="0.01" placeholder="Optional" /></label><label>Next due date<input name="due_date" type="date" /></label><label>Country<select name="country_code" defaultValue="AE"><option value="AE">UAE</option><option value="IN">India</option><option value="US">United States</option><option value="GB">United Kingdom</option></select></label><label>Workspace<select name="workspace_id" required defaultValue={workspaces[0]?.id}>{workspaces.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="full-span">Notes<textarea name="notes" rows={3} placeholder="Interest, lender, repayment context…" /></label></div><div className="modal-actions"><button type="button" className="soft-button" onClick={onClose}>Cancel</button><button className="primary-button"><Check size={15} /> Add debt</button></div></form></ModalShell>; }
