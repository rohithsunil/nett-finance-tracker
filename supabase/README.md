...
2. Run `migrations/0001_nett_mvp.sql` once, then run `migrations/0002_nett_financial_workflows.sql` and `migrations/0003_profile_view_preferences.sql`.
...
Migration 0003 adds per-user country and currency visibility preferences. Nett also keeps a browser-local fallback so the controls remain usable if the migration has not been applied yet.
